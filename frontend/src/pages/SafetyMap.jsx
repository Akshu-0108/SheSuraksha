...
import { CheckCircle2, Download, ExternalLink, Flag, MessageSquare, Save, ShieldAlert, ShieldCheck, Siren, X } from 'lucide-react'
...
function SafetyMap() {
  ...
  const [reportCategory, setReportCategory] = useState('Broken streetlight')
  ...
  const triggerSos = (type) => {
    setSosAlert({
      ...
      message: type === 'silent' ? 'Silent police alert triggered' : 'One-tap SOS broadcast triggered',
      status: 'active',
    })
  }
  ...
  return <main className="safety-map-page min-h-screen bg-[#f4f1e9] text-cream motion-page"><section className="relative h-screen w-screen overflow-hidden">
    ...
    <div className="absolute right-4 top-20 z-[500] flex flex-col gap-2"><button onClick={() => { setSosAlert(null); setSosOpen(true) }} className="flex items-center gap-2 rounded-full bg-red-600 px-3 py-2 text-xs font-bold text-white shadow-lg transition hover:bg-red-700"><Siren size={15} />Emergency SOS</button><button onClick={() => setReportOpen(true)} className="flex items-center gap-2 rounded-full border border-cream/20 bg-[#4c1728]/95 px-3 py-2 text-xs font-semibold text-cream shadow-lg"><Flag size={14} />Report a journey issue</button>{routeOptions.length > 0 && <button onClick={() => setFeedbackOpen(true)} className="flex items-center gap-2 rounded-full border border-gold/40 bg-[#4c1728]/95 px-3 py-2 text-xs font-semibold text-gold shadow-lg"><MessageSquare size={14} />End journey</button>}</div>
    {sosOpen && <SosDialog alert={sosAlert} onClose={() => setSosOpen(false)} onBroadcast={() => triggerSos('broadcast')} onSilentAlert={() => triggerSos('silent')} />}
    {reportOpen && <MapDialog title="Report a journey issue" value={reportMessage} onChange={setReportMessage} placeholder="Tell us where it happened and what made the route feel unsafe…" category={reportCategory} onCategoryChange={setReportCategory} categories={['Broken streetlight', 'Dark patch', 'Suspicious activity', 'Unsafe route condition', 'Other']} onClose={() => setReportOpen(false)} onSubmit={() => { setReportOpen(false); setReportMessage(''); setReportCategory('Broken streetlight'); setSavedMessage('Thank you—your journey issue has been recorded for review.') }} />}
...
function MapDialog({ title, value, onChange, placeholder, category, onCategoryChange, categories, onClose, onSubmit }) {
  return <div className="absolute inset-0 z-[1000] flex items-center justify-center bg-[#16030a]/55 p-4"><form onSubmit={(event) => { event.preventDefault(); onSubmit() }} className="w-full max-w-sm rounded-3xl bg-cream p-5 text-burgundy-dark shadow-2xl"><div className="flex items-center justify-between"><h2 className="font-display text-2xl font-bold">{title}</h2><button type="button" onClick={onClose} aria-label="Close dialog" className="rounded-full p-2 text-burgundy"><X size={18} /></button></div>{categories && <><p className="mt-3 text-sm text-burgundy-dark/70">What did you face during your journey?</p><div className="mt-2 flex flex-wrap gap-2">{categories.map((item) => <button key={item} type="button" onClick={() => onCategoryChange(item)} className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${category === item ? 'border-burgundy bg-burgundy text-cream' : 'border-burgundy/20 bg-white text-burgundy hover:border-burgundy/50'}`}>{item}</button>)}</div></>}<textarea required value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="mt-4 min-h-28 w-full resize-none rounded-2xl border border-burgundy/20 bg-white p-3 text-sm outline-none focus:border-burgundy" /><button className="mt-3 w-full rounded-full bg-burgundy px-4 py-2.5 text-sm font-semibold text-cream">Send report</button></form></div>
}

function SosDialog({ alert, onClose, onBroadcast, onSilentAlert }) {
  return <div className="absolute inset-0 z-[1000] flex items-center justify-center bg-[#16030a]/55 p-4"><section role="dialog" aria-modal="true" aria-labelledby="map-sos-title" className="w-full max-w-sm rounded-3xl bg-gradient-to-br from-burgundy-dark to-[#3a0d10] p-5 text-center text-cream shadow-2xl"><div className="flex justify-end"><button type="button" onClick={onClose} aria-label="Close emergency SOS" className="rounded-full p-2 text-cream/80 hover:bg-white/10"><X size={18} /></button></div><Siren className="mx-auto -mt-1 text-red-300" size={31} /><h2 id="map-sos-title" className="mt-2 font-display text-3xl font-bold">Emergency SOS</h2><p className="mt-2 text-sm text-cream/70">Choose the alert you need. If you are in immediate danger, call 112.</p><div className="mt-5 flex flex-col gap-3"><button onClick={onBroadcast} className="flex items-center justify-center gap-2 rounded-full bg-red-600 py-3.5 text-base font-semibold text-white transition hover:bg-red-700"><Siren size={19} />One-Tap SOS Broadcast</button><button onClick={onSilentAlert} className="flex items-center justify-center gap-2 rounded-full border border-gold/40 py-3 text-sm font-medium text-cream transition hover:bg-black/20"><ShieldAlert size={18} className="text-gold" />Silent Police Alert</button></div>{alert && <div className="mt-5 flex gap-3 rounded-2xl border border-green-500/30 bg-black/30 p-4 text-left"><CheckCircle2 size={20} className="mt-0.5 shrink-0 text-green-400" /><div><p className="text-sm font-semibold">Alert dispatched — status: {alert.status}</p><p className="mt-1 text-xs leading-5 text-cream/65">{alert.message} · Live GPS shared with your emergency circle and nearby verified responders.</p><p className="mt-1 text-[11px] text-cream/40">ID: {alert.sosId}</p></div></div>}</section></div>
}
